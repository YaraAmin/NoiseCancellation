...
min_duration, frame_length, hop_length_frame, hop_length_frame_noise, nb_samples, n_fft, hop_length_fft):
    ...
    list_voice_files=[]
    list_noise_files=[]
    for paths, dirs, files in scandir.walk(noise_dir):
      for file in files:
        if file.endswith(".wav"):
           list_noise_files.append(os.path.join(paths, file))
    for paths, dirs, files in scandir.walk(voice_dir):
      for file in files:
        if file.endswith(".wav"):
          list_voice_files.append(os.path.join(paths, file))
